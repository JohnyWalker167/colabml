...
class TelegramUploader:
    ...
    async def _upload_file(self, cap_mono, file, o_path, force_document=False):
        ...
        try:
            ...
            self.copied_msg = await self._copy_message()
            
            if self.copied_msg and Config.SCREENSHOT_CHAT:
                link = f"https://telegram.dog/tgarchiver3bot?start={self.copied_msg.id}"
                reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Send in DM", url=f'{link}')]])
                SCREENSHOT_CHAT = int(Config.SCREENSHOT_CHAT)
                caption = re.sub(r'\.mkv|\.mp4|\.webm', '', self.copied_msg.caption)
                ss = imgclient.upload(file=thumb, name=caption)
                file_info = {
                    'file_id': self.copied_msg.id,
                    'file_name': caption,
                    'poster_url': ss.url 
                }
            
                await collection.insert_one(file_info)
                self._sent_msg = await TgClient.bot.send_photo(
                    SCREENSHOT_CHAT,
                    photo=thumb,
                    caption=f"<blockquote expandable><b>{caption}</b></blockquote>",
                    disable_notification=True,
    ...
    async def _copy_message(self):
        ...
        async def _copy(target, retries=3):
            for attempt in range(retries):
                try:
                    ...
                    copied_msg = await msg.copy(target)
                    return copied_msg 
                except Exception as e:
        ...
        if self._user_dump:
            with contextlib.suppress(Exception):
                if self._copied_msg is None:
                    self._copied_msg = await _copy(int(self._user_dump))
                return self._copied_msg
        return None
